  ...
  res.send(`
    ...
    <head>
      ...
      <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .clip-item { background: #1a1a1a; border-radius: 10px; padding: 1rem; margin-bottom: 0.75rem; display: flex; justify-content: space-between; align-items: center; }
        .clip-info p { color: white; font-size: 0.95rem; font-weight: bold; }
        .clip-info span { color: #888; font-size: 0.8rem; }
        .btn-descargar { width: auto; padding: 0.5rem 1rem; font-size: 0.85rem; margin: 0; background: #a855f7; border-radius: 8px; border: none; color: white; cursor: pointer; text-decoration: none; display: inline-block; }
        .btn-descargar:hover { background: #9333ea; }
    ...
    <body>
      <div class="container">
        ...
        <div id="clips-lista">
          <h2 style="color:#a855f7; margin-bottom:1rem;">Clips listos para descargar</h2>
      ...
      <script>
        ...
        async function generarClips() {
          ...
          document.getElementById('estado').style.display = 'block';
          document.getElementById('estadoTexto').innerText = 'Descargando y cortando el stream... Esto puede tardar varios minutos.';
          document.getElementById('barraContenedor').style.display = 'block';
          let progreso = 0;
          const intervalo = setInterval(() => {
            if (progreso < 90) { progreso += 2; document.getElementById('barra').style.width = progreso + '%'; }
          }, 3000);
          try {
            const res = await fetch('/generar', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ url: urlActual, duracion: duracionActual })
            });
            const data = await res.json();
            ...
            if (data.error) {
              ...
              data.clips.forEach((clip, i) => {
                contenedor.innerHTML += '<div class="clip-item"><div class="clip-info"><p>Clip #' + (i+1) + '</p><span>Inicio: ' + clip.inicio + ' - Duracion: 34 segundos</span></div><a class="btn-descargar" href="/clips/' + clip.archivo + '" download>Descargar</a></div>';
              });
            }
          } catch(e) {
            clearInterval(intervalo);
...
app.post('/generar', (req, res) => {
  ...
  for (let i = 0; i < totalClips; i++) {
    ...
    clipsInfo.push({
      ...
      archivo: 'clip_' + String(i + 1).padStart(2, '0') + '.mp4'
    });
  }
  ...
  clipsInfo.forEach((clip) => {
    exec('".\\yt-dlp" -g "' + url + '"', (error, stdout) => {
      if (error) {
        completados++;
        if (completados === totalClips) res.json({ clips: clipsInfo });
        return;
      }

      const videoUrl = stdout.trim().split('\n')[0];
      const salidaPath = path.join('clips', clip.archivo);
      const ffmpegCmd = '".\\ffmpeg" -ss ' + clip.inicioSegundos + ' -i "' + videoUrl + '" -t ' + duracionClip + ' -vf "scale=608:1080:force_original_aspect_ratio=decrease,pad=608:1080:(ow-iw)/2:(oh-ih)/2" -c:v libx264 -c:a aac -y "' + salidaPath + '"';

      exec(ffmpegCmd, { timeout: 120000 }, (err) => {
        completados++;
        if (completados === totalClips) res.json({ clips: clipsInfo });
      });
    });
...
app.listen(3000, () => {
  console.log('ClipFlow corriendo en http://localhost:3000');
});
